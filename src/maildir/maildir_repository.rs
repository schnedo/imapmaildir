...
pub struct LocalFlagChanges {
    // todo: do not compact to SequenceSet here, because of change detection
...
impl LocalFlagChangesBuilder {
    fn build(mut self) -> LocalFlagChanges {
        LocalFlagChanges {
            additional_flags: self
                ...
                .map(|(flag, set)| {
                    (
                        flag,
                        set.build().expect("sequence set should not be empty here"),
                    )
                })
                .collect(),
            removed_flags: self
